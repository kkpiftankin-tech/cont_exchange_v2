...
KafkaConsumer::KafkaConsumer(const KafkaConsumerConfig& cfg) {
  ...
  conf->set("max.poll.interval.ms", std::to_string(cfg.max_poll_interval_ms), errstr);

  manual_commit_ = !cfg.enable_auto_commit;
...
bool KafkaConsumer::poll_once(
  ...
  // Manual commit for consumers with enable.auto.commit=false.
  if (manual_commit_) {
    const RdKafka::ErrorCode commit_ec = consumer_->commitSync(msg.get());
    if (commit_ec != RdKafka::ERR_NO_ERROR) {
      log_json("ERROR", "Kafka commitSync failed",
               {{"topic", topic},
                {"partition", std::to_string(msg->partition())},
                {"offset", std::to_string(static_cast<long long>(msg->offset()))},
                {"err", RdKafka::err2str(commit_ec)}});
      return false;
    }
  }
  return true;
