...
static fob::common::v1::Decimal dec_from_double(double x, int32_t scale) {
  // Convert a human number into fixed-point Decimal(units, scale).
  // Example: x=12.34, scale=2 => units=1234
...
static fob::common::v1::Instrument parse_instrument(const std::string& symbol) {
  // Symbol like "BTC/USDT" => base="BTC", quote="USDT".
  ...
  CROW_ROUTE(app, "/v1/flow-orders").methods(crow::HTTPMethod::Post)(
      [this](const crow::request& req) {
        ...
        *order.mutable_instrument() = parse_instrument(symbol);
        ...
        // For MVP we choose scale=8 for quantities and scale=2 for prices by default.
        // In production this must come from instrument meta (tick size/lot size).
        ...
        *meta->mutable_ts_event() = cex::common::now_ts();
        ...
        meta->set_partition_key(order.user_id()); // typical: partition by user

        *grpc_req.mutable_order() = order;

        // ----- Call OrderFlowService -----
