...
- **Exchange-provider** (`providerType=provider`, v2/IN-004) — создаёт заявки через тот же API; после матчинга читает `FillEvent` (`batch.outputs`) и хеджирует позицию через Venue Execution Adapter (F-12).
...
### A3. Exchange-provider создаёт заявку (v2/IN-004, F2-17)
