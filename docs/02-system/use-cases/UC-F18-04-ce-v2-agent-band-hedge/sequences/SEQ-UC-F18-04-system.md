...
Биржа CE как чёрный ящик прогоняет внутренний такт виртуальных контрагентов (A0–A8).
Наружу видно только два факта: клиентская сделка меняет владение биржи НЕМЕДЛЕННО, а
внешняя заявка на площадку уходит ТОЛЬКО когда позиция агента вышла за полосу ±q. Пока
позиция внутри полосы — наружу не видно ничего (интернализация встречного потока).
