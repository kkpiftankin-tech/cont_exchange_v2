...
- **INV-CE-A0:** позицию агента `c_j` двигают только клиентский встречный поток (A0) и
  собственное накопление клиринга, отвечающего на него. При пустом клиентском потоке не
  движется и `c_j` (`Δc_j = 0`) — агенты разных площадок не гоняют встречные позиции друг
  друга автономным арбитражом на разнице мидов; `c_j` не двигает остаток STOCK (его нет).
