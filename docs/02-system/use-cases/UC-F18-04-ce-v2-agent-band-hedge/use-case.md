...
8. **A7 (полоса, эмиссия).** Для каждого агента: если свободная часть позиции (сверх
   покрытого `in_flight`) внутри `|c_j| ≤ q_j` — ничего; иначе отправить наружу
   `(|c_j|−q_j)·sign(c_j)`, размер по стоимости → qty итеративным walk по книге (2–3
   итерации), `PreHedgeCheck` (reuse F-12), пометить отправленное `in_flight`. **Позицию
   `c_j` в момент эмиссии НЕ трогаем** (решение владельца 2026-09-15).
9. **A8 (подтверждения).** Исполнение (в т.ч. частичное) меняет `qty[asset@venue]` узла,
   `realized = fq·(марка−px_факт)/1000·sgn` **И уменьшает `c_j` и `in_flight` на `fq`**.
   После полного исполнения `|c_j| = q_j` точно.
...
- **A-3. Таймаут in_flight.** Неисполненный к дедлайну остаток **снимается с `in_flight`**
  (разрешая повторную отправку в следующем такте); позицию `c_j` таймаут **не** меняет
  (она уменьшалась только по исполнению); realized — только от исполненной доли.
...
- **N-3. PreHedgeCheck REJECT.** Intent не публикуется, `in_flight` НЕ увеличивается
  (позиция и так за полосой, повтор в следующем такте), `risk.alert`.

## Postconditions

- `c_j` обновлён у всех агентов (знаковый, с нуля исторически); при эмиссии позиция не
  меняется (отправленное учтено в `in_flight`), уменьшается по факту исполнения (A8).
