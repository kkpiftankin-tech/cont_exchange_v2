...
### 11. Per-clearing workflow наблюдаемости + канонические решения (2026-09-09)

Целевой сценарий наблюдения (привязка к результату клиринга):

```
клиринг (цены + объёмы) → старая позиция CE → Δ (от клиринга) → новая позиция
   → пороги θ → если |компонента| > θ → планируемый хедж на внешние биржи
```

**Сверка с канонами — верно (dealer/CCP inventory-risk-management), с уточнениями:**

- **(A) Модель контрагента (dealer/CCP).** Позиция от клиринга возникает, т.к.
  биржа — контрагент **нетто-остатка** клиентского потока: `Δ = −(нетто клиентского
  потока батча)`, НЕ валовой объём. Позиция = net, не gross. Внутренне сматченные
  встречные сделки не двигают позицию. *Дефолт: dealer/CCP по residual.*
- **(B) Target-инвентарь и режим хеджа.** Хедж возвращает в **лимит/таргет**, не
  всегда в 0. `Position_a = own_holdings_a − target_a`; `target` default = **seed**
  (биржа держит мультивалютный капитал, хеджит отклонения) — env `CE_TARGET_<CCY>`.
  Режим: **TO_BAND** (к границе θ, меньше churn) default, `FLATTEN` (к таргету) опц.
  Клиентские обязательства — сегрегированная кастодия, в directional-хедж не входят
  (покрыты custody). Numeraire — плавающий target, не хеджируется.
- **(C) Агрегатный лимит.** Помимо per-currency θ — `CE_MAX_AGGREGATE_NOP`
  (Σ|Position·mark| в numeraire) для коррелированного риска (Basel NOP).
- **(D) Mark-to-market.** Компоненты оцениваются по клиринговым ценам (есть) для
  notional-лимитов и equity/PnL.
- **(E) Единая книга, Δ по batch_id.** `старая→Δ→новая` мерится из фактической
  книги (ledger); Δ атрибутируется fills конкретного batch_id (идемпотентно),
  marks на момент клиринга. ledger проводит **house-ногу контрагента** по нетто-
  потоку батча и снимает per-batch снапшот (old/Δ/new).

**Наблюдаемость (§8 дополнение):** под результатом клиринга (вкладка Clearing) —
таблица `старая → Δ клиринга → новая → θ → планируемый хедж` по компонентам,
привязанная к batch_id; источник — ledger per-batch снапшоты + risk θ/размер.

*Дефолты (A dealer/CCP, B target=seed + TO_BAND, C агрегатный лимит, D MTM)
выбраны как каноничные; конфигурируемы, подлежат вето владельца.*
