...
- позиция — накопленное клирингом обещание; уменьшается **по факту исполнения** хедж-заявки
  (A8), а не в момент эмиссии (решение владельца 2026-09-15, см. Conflict Note в
  [ADR-061 §Conflict Note](../03-architecture/adr/ADR-061-ce-v2-agent-position-band-hedge.md));
- **`in_flight` (committed)** — отдельное поле: «отправлено наружу, ещё не исполнено».
  Растёт при эмиссии (A7), убывает по исполнению/таймауту (A8). Нужно для защиты от повторной
  отправки (в v2 committed НЕ исчезает как поле — меняется его смысл, знаковый);
- пока свободная часть позиции (сверх уже покрытого `in_flight`) внутри полосы `|c_j| ≤ q_j`,
  наружу не эмитится ни одной заявки (A7);
...
«ledger уже **освободил** `in_flight` на неисполненный остаток» (A8.3). По новому учёту
(решение владельца 2026-09-15) позиция `c_j` при таймауте **не меняется** — она уменьшалась
только по факту исполнения; таймаут лишь снимает `in_flight`, разрешая повторную отправку в
следующем такте. Без двухшагового перехода повторный sweep снял бы `in_flight` дважды — тот же
idempotency-паттерн, что и guard по `last_batch_id` в `ce_agent_position`. Sweep-запрос A8.1 опирается на частичный индекс
