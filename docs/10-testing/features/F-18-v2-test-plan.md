...
| INV-CE-A0 | **Позицию агента `c_j` двигают ТОЛЬКО клиентский встречный поток (A0) и собственное накопление клиринга, отвечающего на него.** Пустой клиентский поток ⇒ `Δc_j = 0 ∀j` (не только `Z≡0`): агенты разных площадок НЕ гоняют встречные позиции друг друга автономным арбитражом на разнице мидов. Сильнее INV-1 (INV-1 — про владение `Z`, INV-CE-A0 — про позицию `c`). | U13; `agent_band_test::NoClientFlowNoPositionDrift` | A0, A6, ADR-061 §3 |
