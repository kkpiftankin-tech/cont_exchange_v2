...
| INV-6 | **Позиция меняется по ИСПОЛНЕНИЮ, не при эмиссии** (решение владельца 2026-09-15): при `|c|>q` наружу уходит `(|c|−q)·sign(c)` и помечается `in_flight` (позиция НЕ меняется в момент эмиссии); `c_j` уменьшается по мере исполнения `fq` (`c_j−=sign·fq`, `in_flight−=fq`). После **полного** исполнения `|c_j| = q_j` ТОЧНО. | U11; `agent_band_test::PositionDecrementsOnFill`, `FullFillReachesBand` | A7, A8 |
...
| INV-8 | **Таймаут освобождает `in_flight`, позицию `c_j` НЕ меняет** (решение владельца 2026-09-15): неисполненный к дедлайну остаток снимается с `in_flight` (разрешая повторную отправку), а `c_j` остаётся — он уменьшался только по факту исполнения. Поздний филл после таймаута не двоит учёт (idempotency-guard). | U13; `agent_band_test::TimeoutReleasesInFlightPositionUnchanged`, `LateFillNoDoubleCount` | A8 |
