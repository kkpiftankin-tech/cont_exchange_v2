...
**ETH/SOL delta=0 — КОРРЕКТНО (не баг), разобрано 2026-09-14.** Диагностика клиринга
(`CE_CLEARING_DEBUG`, env-gated, per-asset Σ|f| по ногам + спред anchor + марка μ): при
реальных стаканах спред anchor у ETH=0.37‰, SOL=0.53‰ — **в пределах зоны комиссии c≈0.5‰**
⇒ клиринг корректно НЕ торгует (active_edges=0, converged). BTC торгует, т.к. спред=3.42‰
(широкий, из-за venue-выброса ≈−2.5‰ — тонкий coinbase, alphaT≈53). Т.е. алгоритм торгует
актив только когда межвенчурное расхождение > комиссий; эффективные ETH/SOL-рынки (тесные)
арбитража не дают. Форсировать их торговлю = торговать ниже комиссий (убыточно/нереалистично).
Диагностический блок был временно добавлен в `matching_loop::on_ce_clearing_input` под
`CE_CLEARING_DEBUG` и снят после разбора (владелец: «принять как корректное»). Кандидат
на отдельный разбор — фильтрация stale/тонких venue (BTC-выброс), см. ниже.

**Реалистичная симуляция исполнения (ADR-060) — сделано, коммит `2bf75142`.** sim-заявки
матчатся против ленты РЕАЛЬНЫХ публичных сделок (REST recent-trades; WS = NoopCexWsSession,
реального клиента нет). Флаг `VENUES_SIM_MATCH_REAL_TRADES` (dev=1). См. [ADR-060](../03-architecture/adr/ADR-060-venue-sim-fill-vs-real-trades.md).

**Демо-рычаг:** `SIM_VENUE_BIAS_PM_okx=20` (env, только dev, коммит `b152a150`) НЕ действует
в `multi_real` (bias живёт в неиспользуемом sim-адаптере). Реальные цены арбитражны.
