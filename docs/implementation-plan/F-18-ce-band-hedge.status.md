...
| 1a | **Арбитражёры переноса номинала (A_USDT_*) не хеджируются и паркуются** (c_total до ~1253, in_flight=0): у USDT нет base_price → guard в `detect_and_emit_band_breach_locked` пропускает; хедж «USDT/USDT» вырожден. Нужна явная политика: исключить из band ИЛИ хеджировать перенос номинала иначе (сметать в нумерарий на счёт дома) | Высокий | §A3/§«Счёт дома», numeraire freedom |
...
| 9 | **Тесты**: unit на A7-учёт (эмиссия/fill/timeout, частичное), на трёхзонные пороги, на throughput-инвариант (0 падающих PG-операций) | Высокий | §20 CLAUDE.md |
