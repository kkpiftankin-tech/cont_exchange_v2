...
async function getCurvesDataLegacy(leftBoundary, rightBoundary) {
  try {
    ...
    const params = {};
    if (Number.isFinite(leftBoundary)) {
      params.left_boundary_price = leftBoundary;
    }
    if (Number.isFinite(rightBoundary)) {
      params.right_boundary_price = rightBoundary;
    }

    const [demandResponse, supplyResponse] = await Promise.all([
      api.get('/bid-curve', { params }),
      api.get('/ask-curve', { params })
    ]);
