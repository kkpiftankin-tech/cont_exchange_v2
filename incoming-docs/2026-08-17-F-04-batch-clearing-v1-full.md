# F-04. Batch Clearing Cycle — полная спецификация v1 (с диаграммами)

> **Полная версия F-04**, собранная 2026-08-17. База — декодированный текст архива
> [IN-003](2026-05-13-F-04-Batch-Clearing-v1.md) (вся проза/таблицы, восстановленные
> в читаемую кириллицу), **дополненная** оригинальными `mermaid`-диаграммами и
> UI-мокапом из присланного вложения, которые архив IN-003 потерял (переписал прозой).
> Кодировка исправлена (mojibake декодирован); структура и нумерация разделов 1:1.
...
sequenceDiagram
    ...
    loop Каждые t мс
        ...
        S->>S: Определить объём исполнения по каждой FlowOrder
...
| - создание/отмена заявок |     (internal/ | - по SLA/ошибкам |
|                          |      hedge/    |                  |
|                          |      epsilonMM)|                  |
+--------------------------+----------------+------------------+
```
...
sequenceDiagram
    ...
    loop Каждые batchintervalms
        ...
        R->>R: Исполнить post-trade проверки (VaR, margin, alerts)
    end
...
## Приложение. Актуальная реализация cpp/* (из конца исходного вложения)

Диаграмма фактической событийной хореографии сервисов `cpp/*` (venues → Redpanda → matching → ledger/risk/observability → PG/CH → frontend), включая market-data polling, batch-clearing loop, ledger/risk/observability (par), auto-hedge и live-обновление UI.
